...
- **界面已重做（`DEC-…b3.44`，rev 53）**：从「浏览器默认外观」改成有设计系统的界面。**样式单源 = `web/styles.css`**（`index.html` 里已无内联 `<style>`，有测试钉住不许回流），**`app.mjs` 一行未改**（DOM 契约零改动，494 个既有测试的语义未动）。口径：安静的效率工具 + 一个暖调重音；颜色/圆角/间距/时长全部令牌化（组件规则不许写死颜色）；暖纸 `#faf8f5` / 暖黑 `#161513`；单一深青重音 `#0f766e`；**要学的英文词用系统衬线、中文界面用系统无衬线**（零外部字体，因为零外部资源是硬约束）；深色模式走系统偏好。
  ...
  - 验证：`node --test` **509/0**（504 + 新增 6 条样式断言 − 1 个被误收的探针）；截图证据 `docs/ui-redesign/`。**11 档全覆盖**（`TASK-…b3.54` 补验，报告 `docs/ui-redesign/剩余档位验证-2026-09-17.md`）：390×844 下**横溢出元素 0、控制台错误 0**；任务书当时担心的 `feedback` 四颗按钮**不成立**（实测该行只有 2 颗「再写一次 / 下一个词」，而全站最多按钮的动作行是 `reading` 的 3 颗）。**仍未验**：Firefox / WebKit（本机没装）、真机（触屏热区 / 软键盘 / 真视频流）、`:hover`/`:disabled`/`::placeholder` 状态色对比度。
  - **两条待裁决的设计问题**（子代理发现、已独立复核属实，**尚未修**）：**重要 1** `word` 屏要学的词「mug」是**无衬线**，与「要学的英文词用衬线」口径不一致（`--font-word` 只落在候选词与待补原句两处）；**重要 2** 主流程屏（`capturing`/`word`/`composing`）的**唯一推进按钮没有任何重音**——全站只有 ready 的「拍照」与 settings 的「保存」是实心，而那三屏的 `div.row` 祖先链与 `pending`/`feedback` **完全一样**，所以纯 CSS 判据已到极限（这是第四次面对同一个问题）。两条都倾向需要**极小的 DOM 改动**（给标题/主按钮一个类名）才能根治，但那会突破 `DEC-…b3.44` 的「DOM 零改动」口径，需先裁决。
