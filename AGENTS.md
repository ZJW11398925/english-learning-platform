...
- **真机实弹已跑两轮**（决策 `DEC-…13`/`DEC-…15`/`DEC-…21`，摘要副本 `tmp/实机测试-2026-09-15/`）：识物/帧质检/降级/造句判定/uncertain/pending 队列/**复现（识物命中 + 档位推进）**全部真机验证合格。真机缺陷**跟读引擎报错不可见且不落流**已修复（`git:6758e44`，验收 `VAL-…17`/`VR-…19` PASS：引擎报错/超时每会话首条落 `speech_unsupported`，reason=engine_error:<码>/timeout；失败提示升全色文本）。**修复后回读错误码 = aborted**（`DEC-…21`：平台中断识别器，非权限/网络/静默）——该设备+浏览器自动跟读判定不可用且网页侧无法修复，跳过是设计内出口；要跟读判定数据须换 Chrome+GMS+服务可达的安卓机，或 iPhone 开「设置→通用→键盘→听写」后用 Safari。
