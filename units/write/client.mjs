...
//
// ── 思考默认**关**（`thinking: {type:'disabled'}`）—— 本模块唯一的"成本默认"────────
// 依据 TASK F 的实弹对照（`docs/live-write-2026-09-18-thinking-off/`：同一批 6 个回合、同一批草稿、
// 同一段代码、同一时段全空闲价，**只改这一处**）：
//   钱 **0.102750 → 0.014676 元（7.0×）**｜最慢一次 ① **18530 → 2694 ms（6.9×）**｜
//   出 token 24878 → 2583，其中 **reasoning 21882 → 0**｜机械护栏（V1–V5）全过、幸存率 85.7% → 80.0%（持平）。
// 产品负责人裁 `DEC-…OPI-…db.217`。钉住它的断言在 `tests/write-client.test.mjs`
// （「上行形状：思考默认**关**」）——**它是断言，不是注释**：谁把这一行改回去，那一条就变红。
// ⚠️ **一条如实登记的副作用（本轮只登记、不处置）**：官方口径是「思考模式下 `temperature` 不生效」
//   ⇒ 关掉思考之后，下面那行 `temperature: 0.2` **才开始真的生效**，输出风格可能因此变化。
//   本轮**没有**为了"控制风格"去调它（不经授权的默认变更一律不做）；观测口径见 TASK G 报告。
//
...
export async function callModel({
  ...
  const body = {
    ...
    // ⚠️ 它**在思考模式下不生效**；本请求体已把思考默认关掉（见文件头那一节），
    // 所以这一行**现在才开始真的生效**——这一句事实已登记，本轮不动它的取值。
    temperature: 0.2,
    // 思考默认**关**（产品负责人裁 `DEC-…OPI-…db.217`；依据与副作用见文件头那一节）。
    // 位置固定在请求体里、且是**唯一**的思考开关：探针/测试都读这一份真上的 body
    // （`tmp/probes/live-write-check.mjs` 的 `--thinking=omit` 靠"确实删掉了这个字段"证明对照非空）。
