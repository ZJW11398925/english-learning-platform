 ...
 * 分不清模型慢、服务端挂掉还是网络断。
 *
 * 取值依据：实弹探针四次真实调用实测端到端 1079 / 1137 / 1313 / 2129 / 3210 / 3439 ms
 * （task-8-report 实弹探针一节），**最慢一次服务端自报 latency_ms = 3210ms**；
 * 20s 对它约有 6 倍余量，也容得下模型偶尔多写几十个 token 的改写建议
 * （实测 completion_tokens 从 40 到 199 波动——"note 写长一点"就能让一次调用慢下来）。
