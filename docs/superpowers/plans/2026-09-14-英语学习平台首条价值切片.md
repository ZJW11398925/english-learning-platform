...
**Interfaces:**
- Consumes: 无
- Produces: `loadEnv(source)` → `{ VISION_API_KEY, VISION_API_BASE, TEXT_API_KEY, TEXT_API_BASE, PORT }`，缺失必需项时抛错并列出缺哪些
...
const full = {
  VISION_API_KEY: 'k1', VISION_API_BASE: 'https://v.example',
  TEXT_API_KEY: 'k2', TEXT_API_BASE: 'https://t.example', PORT: '8787',
};

test('齐全时解析成功并将 PORT 转成数字', () => {
  const env = loadEnv(full);
  assert.equal(env.PORT, 8787);
  assert.equal(env.VISION_API_KEY, 'k1');
});

test('缺任一项必需变量则抛错，并逐一列出缺失项', () => {
  const { TEXT_API_KEY, ...missing } = full;
  assert.throws(() => loadEnv(missing), /TEXT_API_KEY/);
});
...
test('空字符串等同于缺失（防止 .env 里留空键）', () => {
  assert.throws(() => loadEnv({ ...full, VISION_API_KEY: '' }), /VISION_API_KEY/);
});
```
...
```javascript
// server/env.mjs
const REQUIRED = ['VISION_API_KEY', 'VISION_API_BASE', 'TEXT_API_KEY', 'TEXT_API_BASE'];
...
未填密钥时运行应报错退出：`缺少必需的环境变量: VISION_API_KEY, ...`
...
在 `server/index.mjs` 中用真实视觉模型调用替换 `not_implemented_until_task_8` 分支：把上传的图片转 base64，向 `${VISION_API_BASE}` 发请求，要求模型返回严格 JSON：
