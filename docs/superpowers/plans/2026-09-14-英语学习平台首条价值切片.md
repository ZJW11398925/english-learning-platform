...
test('1 天档完成推进到 3 天档', () => {
  const w = { id: 'w1', stage: 1, createdAt: t0, dueAt: t0 + 1 * DAY };
  assert.equal(nextState(w, t0).dueAt, t0 + 1 * DAY + 3 * DAY);
});
...
export function dueWords(words, now) {
  return Object.values(words).filter((w) => !isMaintained(w) && w.dueAt !== null && w.dueAt <= now);
}
```
