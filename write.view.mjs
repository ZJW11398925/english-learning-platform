 ...
 *   这一条看着像排版偏好，实际是被门量出来的：G3 的「≤13px 的小字」是
 *   **逐文本元素**数的，与折不折行无关。把每一项做成 `<span>` 子元素时实测
 *   小字 15 处（门红）；改成"一个元素 + 文本 `·` 分隔"之后降下来 ——
 *   **这一屏（`wordcard`）今天实测 6 处**（`docs/ui-rebuild/gates.json`）。
 *   每个字段**仍带一小段自己的标记类**（如 `pos` / `牛津3000` / `词频 极高`），
 *   探针与门照样能逐项读，不靠子元素。
 *
 * 字号：这一行是 **`--t-2`（16px）**，由人裁决放宽（2026-09-19）——理由是
 *   "这一屏的内容就是这张卡"，而 11px 的音标/词性在手机上偏小。16px 是这一屏**已有**的
 *   档位 ⇒ 没有新增字号档（实测整屏仍 36/22/16/11 四档）。样式与边界写在 `write.css`
 *   的 `.wc-meta` 那一段（含"本页其它 11px 一律不动"）。
 */
...
function wordCard(doc, card, handlers) {
  ...
  if (lex !== null) {
    ...
    // ⚠️ 这两件合成**一个文本元素**（"变形：… ｜ 例句与搭配：暂缺"一行）。
    //    当初合成它们是被配额逼的（逐元素数，拆成两个就多一处小字）。
    //    **今天这一条已经不再被配额逼着**（如实写清，免得下一个人以为拆开就会红）：
    //    元信息行提到 16px 之后，这一屏实测 **5 处** ≤13px 的小字，而 `wordcard`
    //    这一屏的预算已由人裁决放宽到 **8**（其余 15 屏仍是 7，见 `scripts/ui-gates.mjs`）。
    //    保持合成是因为"这是同一栏的两句话"读起来更整，不是为了躲配额。
    const notes = [];
